...
export default function ViewPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const content = isPdf ? "" : getDecodedContent();
        ...
        <div className="flex justify-center mt-4">
          <button
            onClick={async () => {
              ...
              if (isPdf) {
                // Generate a proper PDF from redacted text
                ...
                // Replace block characters with [REDACTED] for PDF compatibility
                const safeContent = content.replace(/█+/g, "[REDACTED]");
                ...
                for (const line of lines) {
                  ...
                  if (currentLine) {
                    // Draw [REDACTED] parts in red
                    if (currentLine.includes("[REDACTED]")) {
                      page.drawText(currentLine, { x: margin, y, size: fontSize, font, color: rgb(0.8, 0, 0) });
                    } else {
                      page.drawText(currentLine, { x: margin, y, size: fontSize, font, color: rgb(0, 0, 0) });
                    }
                  }
                ...
                downloadName = `redacted-${baseName}.pdf`;
              } else {
                blob = new Blob([content], { type: "text/plain" });
                downloadName = `redacted-${baseName}.txt`;
              }
