  ...
  const handleShare = async () => {
    ...
    try {
      ...
      if (isPdf) {
        ...
        // For text/image files, use text-based redaction
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <div className="bg-slate-900 rounded-lg p-4 max-h-64 overflow-y-auto overflow-x-hidden font-mono text-sm leading-relaxed break-all whitespace-pre-wrap">
          {getHighlightedText().map((part, i) =>
            part.isRedacted ? (
              <span
                key={i}
                className="bg-red-500/30 text-red-300 px-0.5 rounded inline break-all"
                title="Will be redacted"
              >
                {part.text}
              </span>
            ) : (
              <span key={i} className="text-slate-300 break-all">
                {part.text}
              </span>
            )
          )}
